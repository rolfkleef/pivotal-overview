<?php
	...
	class pivotal {
		...
		public function addLabels($story, $labels) {
			...
			// Create the new task
			$cmd = "curl -H \"X-TrackerToken: {$this->token}\" "
				 . "-s -X PUT -H \"Content-type: application/xml\" "
		...
		// getCurrentStories
		// -----
		// Get a list of stories from a project in the current iteration
		public function getCurrentStories($project) {
		...
		// getIterations
		// -----
		// Get a list of all iterations and stories from a project
		public function getIterations($project) {
			...
			$cmd = "curl -H \"X-TrackerToken: {$this->token}\" "
				 . "-s -X GET "
				 . "https://www.pivotaltracker.com/services/v3/projects/$project/iterations";
			$xml = shell_exec($cmd);
